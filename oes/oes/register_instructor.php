...
if ($fullName && $email && $instructorId && $password && $confirmPassword) {
    if ($password === $confirmPassword) {
        ...
        $sql = "SELECT email, instructor_id FROM instructor_requests WHERE email = ? OR instructor_id = ? 
                UNION 
                SELECT email, instructor_id FROM registered_instructors WHERE email = ? OR instructor_id = ?";
